...
def parse_config(config_file):
    ...
    arg.fit = ArgObject()
    arg.fit.nb_replicates = config.getint("hyperparams",
            "nb_replicates", fallback=2)
    arg.fit.elbo_type = config.get("hyperparams",
            "elbo_type", fallback="elbo")
    arg.fit.grad_samples = config.getint("hyperparams",
            "grad_samples", fallback=1)
    arg.fit.K_grad_samples = config.getint("hyperparams",
            "K_grad_samples", fallback=0)
    arg.fit.nb_samples = config.getint("hyperparams",
            "nb_samples", fallback=10)
    arg.fit.alpha_kl = config.getfloat("hyperparams",
